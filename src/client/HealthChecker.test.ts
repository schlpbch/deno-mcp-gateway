...
Deno.test(
  'HealthChecker - returns HEALTHY when actuator returns 200',
  async () => {
    setupMockFetch();
    try {
...
Deno.test(
  'HealthChecker - constructs correct actuator URL for /mcp endpoint',
  async () => {
    setupMockFetch();
    try {
...
Deno.test(
  'HealthChecker - constructs correct actuator URL for root endpoint',
  async () => {
    setupMockFetch();
    try {
      mockFetchResponses.set('https://api.example.com/actuator/health', {
...
Deno.test(
  'HealthChecker - falls back to MCP when actuator returns 404',
  async () => {
    setupMockFetch();
    try {
...
Deno.test(
  'HealthChecker - falls back to MCP when actuator throws error',
  async () => {
    setupMockFetch();
    try {
      // MCP returns 200
...
Deno.test(
  'HealthChecker - MCP fallback stores session ID when returned',
  async () => {
    setupMockFetch();
    try {
      // Actuator fails
      ...
      assertEquals(
        sessionManager.getSession('server-1'),
...
Deno.test(
  'HealthChecker - returns DEGRADED when MCP returns non-OK response',
  async () => {
    setupMockFetch();
    try {
      // Actuator fails
...
Deno.test(
  'HealthChecker - includes correct headers in MCP request',
  async () => {
    setupMockFetch();
    try {
      mockFetchResponses.set('https://example.com/actuator/health', {
      ...
      const mcpCall = fetchCalls.find(
        (c) => c.url === 'https://example.com/mcp'
      );
...
Deno.test(
  'HealthChecker - increments consecutiveFailures on MCP failure',
  async () => {
    setupMockFetch();
    try {
      mockFetchResponses.set('https://example.com/actuator/health', {
...
Deno.test(
  'HealthChecker - increments consecutiveFailures on network error',
  async () => {
    setupMockFetch();
    try {
      globalThis.fetch = async () => {
